...
### Example Usage
In this paper, the authors proposed `PrefixCache` and `DualCache`. To use `PrefixCache`:
```bash
# Test Fast-dLLM on HumanEval with LLaDA-7B-Base, run:
accelerate launch \
    --num_machines 1 \
    --num_processes 4 \
    eval.py \
...
```
To use `DualCache`, add `cache.use_dual=true` to commands, which also caches tokens after the current decoding lock.
...
### Example Usage
There are three key hyperparameters that control the behavior of `dLLMCache`:
```yaml
kr: null # update interval for response part
kp: null # update interval for prompt part
rou: 0.25 # adaptive update ratio for response tokens
```
To use `dLLMCache`:
```bash
...
```yaml
rollout_p: 0.1 # update top-p tokens that aggregate most attention
current_k: 32 # # of tokens to update in masked tokens
sigma: 10.0 # sigma in Gaussian function in Eq(3)
