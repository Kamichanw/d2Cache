...
def run_apply_delta_test():
    """
    A comprehensive test case for the Frame.apply_delta method.
    (with corrected expected values)
    """
    print("--- Running Comprehensive Test for apply_delta ---")

    # --- 1. Setup Initial State ---
    os.environ["MASK_TOKEN_ID"] = "0"
    initial_frame = Frame.create_initial_frame(
        prompts=torch.arange(12, dtype=torch.long).reshape(4, 3),
        gen_length=6,
        mask_token_id=0,
    )
    ...
    delta = FrameDelta(
        ...
        insert_index=torch.tensor([[0, 3], [2, 5]]),
        insert_src_index=torch.tensor([[3, 4], [0, 1]]),
        delete_index=torch.tensor([[2], [0]]),
    )

    # --- 3. Manually Calculate CORRECTED Expected Output ---

    # After Insert for Seq 0: [13, 0, 10, 0, 14, 0, 11, 0] -> After Delete[2]: [13, 0, 0, 14, 0, 11, 0]
    # After Insert for Seq 2: [0, 0, 20, 0, 0, 0, 21, 22] -> After Delete[0]: [0, 20, 0, 0, 0, 21, 22]
    expected_generated_tokens = torch.tensor(
        [
            [13, 0, 0, 14, 0, 11, 0],
            [0, 0, 0, 0, 0, 0, 0],  # Inactive, padded to final length
            [0, 20, 0, 0, 0, 21, 22],
            [0, 0, 0, 0, 0, 0, 0],  # Inactive, padded to final length
        ]
    )

    # Steps for Seq 0: [-1,0,-1,-1,0,-1] -> Insert [0,0]: [0,-1,0,-1,0,-1,0,-1] -> Delete[2]: [0,-1,-1,0,-1,0,-1]
    # Steps for Seq 2: [-1,-1,-1,-1,-1,0] -> Insert [0,0]: [-1,-1,0,-1,-1,-1,0,0] -> Delete[0]: [-1,0,-1,-1,-1,0,0]
    ...
    print("\nExpected Final Tokens (Corrected):\n", expected_generated_tokens)
    print("Expected Final Steps (Corrected):\n", expected_steps)

    # --- 4. Run the function and get the actual output ---
    new_frame = initial_frame.apply_delta(delta)

    print("\nActual Final Tokens:\n", new_frame.generated_tokens)
    print("Actual Final Steps:\n", new_frame.steps)

    # --- 5. Assert that the actual output matches the expected output ---
    try:
        torch.testing.assert_close(
            new_frame.generated_tokens, expected_generated_tokens
        )
        torch.testing.assert_close(new_frame.steps, expected_steps)
        print("\n✅✅✅ TEST PASSED! ✅✅✅")
...
if __name__ == "__main__":
    # To run this test, you need to have all the class definitions
    # (Base, Intermediate, FrameDelta, Frame) in the same script or imported.
    run_apply_delta_test()
